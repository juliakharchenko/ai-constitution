...
Provide the report in a clear, structured format with sections for each part. Ensure the analysis is objective, precise, and evidence-based.`;
  ...
  //   analysisResponse: string, // Renamed parameter to avoid confusion with DOM Response
  //   principles: string[],
  //   userValues: { value: string; weight?: number }[],
  //   responsibleAIFrameworks: { framework: string; weight?: number }[]
  // ): AnalysisReport {
  //   // Attempt to parse structured response; fallback to heuristic if parsing fails
  //   try {
  //     // Assuming the LLM returns a structured text response with sections
  ...
  //         continue;
  //       } else if (line.startsWith('## Adherence Analysis')) {
  //         currentSection = 'adherence';
  //         continue;
  //       } else if (line.startsWith('## Non-Adherence Analysis')) {
  //         currentSection = 'nonAdherence';
  //         continue;
  //       } else if (line.startsWith('## Weighted Score')) {
  ...
  //           summary += (summary ? '\n' : '') + line;
  //         } else if (currentSection === 'adherence' && line.includes(':')) {
  //           const [item, ...descParts] = line.split(':');
  //           adherence.push({ item: item.trim(), description: descParts.join(':').trim() });
  //         } else if (currentSection === 'nonAdherence' && line.includes(':')) {
  //           const [item, ...descParts] = line.split(':');
  //           nonAdherence.push({ item: item.trim(), description: descParts.join(':').trim() });
  //         } else if (currentSection === 'score' && line.match(/Score: \d*\.?\d+/)) {
  ...
  //     return {
  //       summary: analysisResponse || 'No summary provided by the analysis.',
  //       adherenceAnalysis: adherence.length > 0 ? adherence : [{ item: 'General', description: 'No specific adherence details provided.' }],
  ...
  //     return this.fallbackHeuristicAnalysis(analysisResponse, principles, userValues, responsibleAIFrameworks);
  //   }
  // }
  private parseAnalysisResponse(
