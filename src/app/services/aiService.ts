...
export class MultiProviderAIService {
  ...
  private loadStoredConfig() {
    // Load API keys and selected models from memory (not localStorage)
  ...
  setSelectedModels(selectedModels: SelectedModels) {
  ...
  getConfiguredProviders(): AIProvider[] {
    return AI_PROVIDERS.filter(provider => 
      this.apiKeys[provider.id] && this.apiKeys[provider.id].trim() !== ''
    );
  }

  getSelectedModels(): { provider: AIProvider; model: AIModel }[] {
    const results: { provider: AIProvider; model: AIModel }[] = [];
    
    Object.entries(this.selectedModels).forEach(([providerId, modelId]) => {
...
- Be 2-3 paragraphs long`;
  ...
  async analyzeAlignment(response: string, principles: string[]): Promise<AlignmentAnalysis> {
    // For now, use the existing heuristic analysis
    // You could extend this to use different models for analysis
