...
// use this if the rest doesnt work
// 'use client';
// import React, { useState, useEffect } from 'react';
...
// import { HofstedeDimensions } from '../types';
// import { QuestionnaireQuestion } from '../types';
// import { QuestionnaireCategory, SelectedModel, AIResponse, PersonalityResponse, ConstitutionalAI, MultiProviderAI, TrustTemplate, AlignmentAnalysis, AnalysisReport } from '../types/ai';
// import { Tooltip } from 'react-tooltip';
// import { useLogging } from '../components/LoggingProvider/LoggingProvider';
// import { LoggingProvider } from '../components/LoggingProvider/LoggingProvider';
...
//       <div>
//         <label className="block text-sm font-medium text-gray-700 mb-2">
//           Add Custom Responsible AI Principle
//         </label>
//         <div className="flex gap-2">
//           <input
//             type="text"
...
// //     if (!response) {
// //       throw new Error('No response generated for the selected model');
// //     }

// //     const analysis = await multiProviderAI.analyzeAlignment(
// //       response.response,
// //       constitution,
// //       constitution.map(p => ({ value: p, weight: 1.0 })),
// //       effectiveSafetyDimensions.map(f => ({ framework: f, weight: weights.safety / 100 }))
// //     );
...
// //         criteria.forEach(criterion => {
// //           if (criterion.minAlignment && analysis.alignmentScore < criterion.minAlignment) {
// //             passed = false;
// //             details.push(`Failed ${criterion.name}: Alignment score ${(analysis.alignmentScore * 100).toFixed(1)}% < ${(criterion.minAlignment * 100).toFixed(0)}%`);
// //           }
// //           if (criterion.minSafety && analysis.alignmentScore < criterion.minSafety) {
// //             passed = false;
// //             details.push(`Failed ${criterion.name}: Safety score ${(analysis.alignmentScore * 100).toFixed(1)}% < ${(criterion.minSafety * 100).toFixed(0)}%`);
// //           }
// //           if (criterion.requiredDimensions) {
// //             criterion.requiredDimensions.forEach(dim => {
// //               if (!analysis.adherenceAnalysis.some(a => a.item === dim) || analysis.alignmentScore < 0.8) {
// //                 passed = false;
// //                 details.push(`Failed ${criterion.name}: Dimension "${dim}" score too low or missing`);
// //               }
// //             });
...
// //         trust = { score: passed ? 1 : 0, weights: { alignment: 0, safety: 0 }, criteria: { name: trustTemplate === 'custom' ? 'Custom' : trustTemplates[trustTemplate].name, passed, details, explanation } };
// //       } else {
// //         const trustScore = analysis.alignmentScore * (weights.alignment / 100) + analysis.alignmentScore * (weights.safety / 100);
// //         trust = { score: trustScore, weights: { ...weights } };
// //       }
// //     }
...
// //       alignment: analysis,
// //       safety: analysisMode === 'safety' || analysisMode === 'both' ? {
// //         overallScore: analysis.alignmentScore,
// //         dimensions: effectiveSafetyDimensions.reduce((acc, dim) => ({
// //           ...acc,
// //           [dim]: {
// //             score: analysis.adherenceAnalysis.some(a => a.item === dim) ? analysis.alignmentScore : 0.6,
// //             passes: analysis.adherenceAnalysis.filter(a => a.item === dim).map(a => a.description),
// //             concerns: analysis.nonAdherenceAnalysis.filter(a => a.item === dim).map(a => a.description),
// //           },
// //         }), {} as Record<string, { score: number; passes: string[]; concerns: string[] }>),
// //       } : null,
// //       trust,
...
// //       personality: activePersonality,
// //       response: `Sorry, I couldn't generate a response due to an error.`,
// //       alignment: { summary: 'Error occurred', adherenceAnalysis: [], nonAdherenceAnalysis: [{ item: 'General', description: 'Error occurred during analysis' }], alignmentScore: 0, recommendations: [] },
// //       safety: null,
// //       trust: null,
// //       processingTime: 0,
...
//   //   ? personalityResponses.reduce((sum, r) => sum + (r.alignment?.alignmentScore || 0), 0) / personalityResponses.length
...
//             <h2 className="text-2xl font-semibold mb-4 flex items-center">
//               <Shield className="mr-2 text-indigo-600" />
//               Step 2b: Define Responsible AI Frameworks
//               <Info className="w-5 h-5 ml-2 text-gray-500 cursor-pointer" data-tooltip-id="safety-assessment" />
//             </h2>
...
import { hofstedeToPrinciples } from '../lib/hofstedeHelpers';
import type { LucideIcon } from 'lucide-react';
import { HofstedeDimensions, QuestionnaireQuestion } from '../types';
...
const SafetyFrameworksBuilder: React.FC<SafetyFrameworksBuilderProps> = ({ selectedFrameworks, setSelectedFrameworks, safetyPrinciples, setSafetyPrinciples }) => {
  ...
  const handleFrameworkToggle = (frameworkId: string) => {
    if (selectedFrameworks.some(f => f.id === frameworkId)) {
      ...
      const newFrameworks = [...selectedFrameworks, { id: frameworkId, weight: 100 / (selectedFrameworks.length + 1) }];
      setSelectedFrameworks(newFrameworks.map(f => ({
        ...f,
        weight: 100 / newFrameworks.length
      })));
    }
  };
  ...
  const handleWeightChange = (frameworkId: string, weight: number) => {
    ...
    setSelectedFrameworks(
      selectedFrameworks.map(f => ({
        ...f,
        weight: f.id === frameworkId ? normalizedWeight : otherWeight
      }))
    );
  };
...
const UnifiedAIExplorer: React.FC = () => {
  ...
  const [showSetup, setShowSetup] = useState<boolean>(false);
  const [scenario, setScenario] = useState<string>('');
  const [usePersonalities, setUsePersonalities] = useState<boolean>(true);
  const [selectedPersonalities, setSelectedPersonalities] = useState<string[]>([]);
  const [personalityResponses, setPersonalityResponses] = useState<PersonalityResponse[]>([]);
  const [isGeneratingPersonalities, setIsGeneratingPersonalities] = useState<boolean>(false);
  const [analysisMode, setAnalysisMode] = useState<'values' | 'safety' | 'both'>('both');
  ...
  const generateResponseWithPersonality = async (
    ...
    try {
      ...
      const response = responses.find((r: AIResponse) => r.modelId === model.id);
      if (!response) {
        throw new Error('No response generated for the selected model');
      }
      ...
      if (analysisMode === 'both') {
        if (useTrustTemplate) {
          ...
          criteria.forEach(criterion => {
            ...
            if (criterion.requiredDimensions) {
              criterion.requiredDimensions.forEach(dim => {
                const adherence = analysis.adherenceAnalysis.find((a: { item: string }) => a.item === dim);
                if (!adherence || analysis.alignmentScore < 0.8) {
                  passed = false;
                  details.push(`Failed ${criterion.name}: Dimension "${dim}" score too low or missing`);
                }
              });
      ...
      return {
        ...
        safety: analysisMode === 'safety' || analysisMode === 'both' ? {
          ...
          dimensions: effectiveSafetyDimensions.reduce((acc, dim) => ({
            ...acc,
            [dim]: {
              score: analysis.adherenceAnalysis.find((a: { item: string }) => a.item === dim) ? analysis.alignmentScore : 0.6,
              passes: analysis.adherenceAnalysis.filter((a: { item: string; description: string }) => a.item === dim).map((a: { description: string }) => a.description),
              concerns: analysis.nonAdherenceAnalysis.filter((a: { item: string; description: string }) => a.item === dim).map((a: { description: string }) => a.description),
            },
          }), {} as Record<string, { score: number; passes: string[]; concerns: string[] }>),
        } : null,
        trust,
      ...
      {personalityResponses.length > 0 && (
        <div className="mt-8">
          ...
          <MultiProviderResults
            ...
            onAnalyzeAlignment={async (response: PersonalityResponse) => {
              const analysis = await multiProviderAI.analyzeAlignment(
                response.response,
                constitutionalAI.constitution,
                constitutionalAI.constitution.map(p => ({ value: p, weight: 1.0 })),
                (analysisMode === 'safety' || analysisMode === 'both')
                  ? selectedFrameworks.map(f => ({ framework: extendedSafetyTemplates[f.id].name, weight: f.weight / 100 }))
                  : []
              );
            ...
            alignmentAnalyses={personalityResponses.reduce((acc: { [modelId: string]: AnalysisReport }, r) => {
              if (r.alignment) {
                acc[r.modelId] = r.alignment;
              }
              return acc;
