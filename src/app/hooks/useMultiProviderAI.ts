...
export function useMultiProviderAI() {
  ...
  const getProviderName = useCallback((modelId: string) => {
    // Find the provider associated with the modelId in selectedModels
    for (const [providerId, selectedModelId] of Object.entries(selectedModels)) {
      if (selectedModelId === modelId) {
        const provider = providers.find(p => p.id === providerId);
